...
from models.models import SenhaFracaError, Usuario, db
...
def esqueci_senha():
    if request.method == 'POST':
        ...
        msg_padrao = (
            'Se o e-mail estiver cadastrado, um link de reset foi gerado. '
            'Em produção este link seria enviado por e-mail.'
        )
        if usuario:
            token = secrets.token_urlsafe(32)
            usuario.token_reset = token
            usuario.token_reset_expira_em = datetime.utcnow() + timedelta(
                hours=TOKEN_VALIDADE_HORAS
            )
            db.session.commit()
            ...
            if email_configurado():
                ...
                try:
                    ...
                    flash(
                        f'{msg_padrao} (Falha no envio; link de desenvolvimento: {link})',
                        'info',
                    )
            else:
                # Sem SMTP configurado: fallback dev mostra o link
                flash(
                    f'{msg_padrao} Para fins de desenvolvimento, o link é: {link}',
                    'info',
                )
        else:
            flash(msg_padrao, 'info')
