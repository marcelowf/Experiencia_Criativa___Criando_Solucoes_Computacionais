    ...
    with db.engine.begin() as conn:
        ...
        if insp.has_table('versoes_pesos'):
            ...
            if 'criada_por_id' in vcols and 'criado_por_id' not in vcols:
                conn.execute(
                    text('ALTER TABLE versoes_pesos RENAME COLUMN criada_por_id TO criado_por_id')
                )
...
def migrar_responsavel_string_para_tabela():
    ...
    from models.models import Paciente, Responsavel, db
    ...
    pendentes = (
        Paciente.query.filter(Paciente.id_responsavel.is_(None))
    ...
    for p in pendentes:
        ...
        if resp is None:
            resp = Responsavel.query.filter(db.func.lower(Responsavel.nome) == chave).first()
        if resp is None:
