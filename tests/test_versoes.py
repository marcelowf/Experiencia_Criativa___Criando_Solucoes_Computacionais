...
from models.models import Avaliacao, Sintoma, SintomaPesoVersao, VersaoPesos
...
def test_alteracao_peso_cria_v2(auth_client, app, db):
    ...
    with app.app_context():
        ...
        snap = SintomaPesoVersao.query.filter_by(id_versao=ativa.id, id_sintoma=sid).first()
        assert snap.peso_masculino == 0.05
        # Snapshot V1 deve manter o peso original (0.01)
        snap_v1 = SintomaPesoVersao.query.filter_by(id_versao=v1.id, id_sintoma=sid).first()
...
def test_novo_sintoma_cria_v2(auth_client, app, db):
    ...
    with app.app_context():
        ...
        snap = SintomaPesoVersao.query.filter_by(id_versao=ativa.id, id_sintoma=s.id).first()
