...
from models.models import (db, EmailConfig, Avaliacao, QrCadastroToken,
                           Responsavel, Usuario, LogAuditoria)
...
def _criar_config(senha_plain='senha-de-app-1234', ativo=True):
    ...
    c = EmailConfig(remetente_email='envio@gmail.com',
...
def test_decifrar_token_invalido_levanta(app):
    from controllers.email_service import decifrar_senha, SenhaAppInvalidaError
    with app.app_context():
...
def test_salvar_config_admin(auth_client, app):
    r = auth_client.post('/config/email/', data={
...
def test_senha_em_branco_mantem_atual(auth_client, app):
    ...
    auth_client.post('/config/email/', data={
...
def test_enviar_sem_config_levanta_erro(db):
    from controllers.email_service import enviar_email, EmailNaoConfiguradoError
...
def test_reset_envia_email_quando_configurado(client, app):
    ...
    with patch.object(reset_senha_controller, 'enviar_email') as mock_env:
        r = client.post('/esqueci-senha', data={'email': 'admin@admin.com'},
                        follow_redirects=True)
        assert r.status_code == 200
...
def test_qr_enviar_email(auth_client, app):
    ...
    with patch.object(qr_cadastro_controller, 'enviar_email') as mock_env:
        r = auth_client.post(f'/pacientes/qr/{qr_id}/enviar-email',
    ...
    assert LogAuditoria.query.filter_by(acao='EMAIL_ENVIADO',
                                        entidade='qr_cadastro_token').count() == 1
...
def _criar_avaliacao_com_responsavel(admin, email_resp='mae@x.com'):
    ...
    db.session.add(resp)
    db.session.flush()
    p = Paciente(nome='Filho', cpf='111.444.777-35', sexo='M',
                 data_nascimento=date(2015, 1, 1),
                 id_responsavel=resp.id, id_usuario=admin.id)
    db.session.add(p)
    db.session.flush()
    av = Avaliacao(id_paciente=p.id, data=date.today(), score=0.5,
                   recomendacao='ENCAMINHAR', id_usuario=admin.id,
...
def test_resultado_enviar_email_anexa_pdf(auth_client, app, admin):
    ...
    with patch.object(avaliacao_controller, 'enviar_email') as mock_env, \
         patch.object(avaliacao_controller, '_resultado_pdf_bytes',
                      return_value=b'%PDF-fake') as mock_pdf:
        r = auth_client.post(f'/avaliacoes/{av_id}/enviar-email',
...
def test_email_destino_paciente_proprio(db, admin):
    ...
    from models.models import Paciente
    p = Paciente(nome='Adulto', cpf='111.444.777-35', sexo='M',
                 data_nascimento=date(1990, 1, 1),
                 email='adulto@exemplo.com', id_usuario=admin.id)
    db.session.add(p)
    db.session.commit()
...
def test_email_destino_fallback_responsavel(db, admin):
    ...
    db.session.add(resp)
    db.session.flush()
    p = Paciente(nome='Crianca', cpf='111.444.777-35', sexo='F',
...
def test_email_destino_paciente_prevalece_sobre_responsavel(db, admin):
    ...
    db.session.add(resp)
    db.session.flush()
    p = Paciente(nome='Adolescente', cpf='111.444.777-35', sexo='M',
...
def test_email_destino_vazio_quando_ninguem_tem(db, admin):
    from controllers.email_service import email_destino
    from models.models import Paciente
    p = Paciente(nome='Sem Email', cpf='111.444.777-35', sexo='M',
                 data_nascimento=date(1990, 1, 1), id_usuario=admin.id)
    db.session.add(p)
    db.session.commit()
...
def test_cadastro_paciente_grava_email(auth_client, app):
    ...
    auth_client.post('/pacientes/novo', data={
        'nome': 'Com Email', 'cpf': '11144477735', 'sexo': 'M',
...
def test_resultado_sugere_email_do_paciente_quando_existe(auth_client, app, admin):
    ...
    from controllers.versoes_pesos import versao_ativa
    from models.models import Paciente, Avaliacao as Av
    resp = Responsavel(nome='Mae', email='mae@x.com')
    db.session.add(resp)
    db.session.flush()
    p = Paciente(nome='Pac', cpf='111.444.777-35', sexo='M',
                 data_nascimento=date(2010, 1, 1),
                 email='paciente@x.com',
                 id_responsavel=resp.id, id_usuario=admin.id)
    db.session.add(p)
    db.session.flush()
    av = Av(id_paciente=p.id, data=date.today(), score=0.5,
            recomendacao='ENCAMINHAR', id_usuario=admin.id,
            id_versao_pesos=versao_ativa().id)
    db.session.add(av)
    db.session.commit()
